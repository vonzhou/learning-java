...
注意:ThreadLocalMap的key是ThreadLocal.

Thread中的threadLocals就用来保存这些线程私有的数据.
...
`ThreadLocalMap`的key(即`ThreadLocal`)为弱引用，所以当其被回收时，需要将value置为null以便于进行垃圾回收。比如remove的时候有调用`java.lang.ref.Reference.clear`方法.
...
此外当使用`InheritableThreadLocal`时,保存的变量不在是threadLocals,而是inheritableThreadLocals.
